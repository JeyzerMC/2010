...
public class Main 
{
	...
	public static void main(String[] args) 
	{
		...
		wName = "Edsger Dijkstra (original)";
		...
		hpmp.zoomIn(0, 0, 2);
		hpmp.resize(hpmp.height/2, hpmp.width/2);
		int[] tMin = new int[3];
		tMin[0] = 110;
		tMin[1] = 110;
		tMin[2] = 110;
		...
		gpmp.zoomIn(0, gpmp.width, 2);
		gpmp.resize(gpmp.height/2, gpmp.width/2);
		gpmp.convertToGrayImage();


		PixelMapPlus bwpmp = new PixelMapPlus( pmp );
		bwpmp.zoomIn(pmp.width, 0, 2);
		bwpmp.resize(bwpmp.height/2, bwpmp.width/2);
		bwpmp.convertToBWImage();
		

		PixelMapPlus npmp = new PixelMapPlus( pmp );
		npmp.zoomIn(npmp.width, npmp.height, 2);
		npmp.resize(npmp.height/2, npmp.width/2);
		npmp.negate();
